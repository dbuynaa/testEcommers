...
const ProductSmallCard = ({ wholeProduct, isFirst, onComplete }) => {
  ...
  return (
    <div key={wholeProduct._id} className={`relative ${isFirst && !isFinished ? "h-full w-full" : "h-56 w-full"} flex flex-wrap gap-10 pb-10 `}>
      <Link
        href={{
      ...
      >
        <div className="flex justify-center bg-slate-600">
          <Image
            ...
            className={`${isFirst && !isFinished ? "w-full h-full object-cover" : ""} hover:scale-105 transition duration-100 cursor-pointer ease-in pt-3 pb-3 `}
            fill={false}
            alt="name"
        ...
        <div className="flex flex-wrap flex-col gap-3 justify-center pt-32 ">
          <span className="text-center pt-1 pl-2 text-xs">{wholeProduct.name}</span>
